...
uint32_t s_box(uint32_t x)
{
    ...
    return s1[x1] | (s1[x2] << 8) | (s1[x3] << 16) | (s1[x4] << 24);
}

uint32_t f_function(uint32_t r, uint32_t k)
{
    return s_box(r ^ k);
}
...
uint32_t encrypt(uint32_t plaintext, uint32_t key)
{
    ...
    left = right;
    uint32_t f_result = f_function(right, key);
...
uint32_t decrypt(uint32_t ciphertext, uint32_t key)
{
    ...
    right = left;
    uint32_t f_result = f_function(left, key);
...
int main()
{
    ...
    uint32_t decrypted = decrypt(ciphertext, key);
